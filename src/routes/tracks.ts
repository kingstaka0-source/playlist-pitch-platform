...
tracks.get("/tracks/:id", async (req, res) => {
  try {
    ...
    const owned = await requireOwnedTrack(id, artistId);
    ...
    console.error("TRACK DETAIL ERROR", message);
...
tracks.post("/tracks/manual", async (req, res) => {
  try {
    ...
    const track = await prisma.track.create({
      data: {
        artistId,
        spotifyTrackId: `manual:${crypto.randomUUID()}`,
        title: safeTitle,
        artists: [],
        durationMs: 180000,
        audioFeatures: {
    ...
    console.error("TRACK MANUAL ERROR", message);
...
tracks.get("/tracks/:id/placements", async (req, res) => {
  try {
    ...
    console.error("TRACK_PLACEMENTS_FAILED", message);
  ...
  /**
 * GET /tracks/:trackId/campaign-history
 */
tracks.get("/tracks/:trackId/campaign-history", async (req, res) => {
  ...
  where: {
    ...
    console.error("CAMPAIGN HISTORY ERROR", message);
